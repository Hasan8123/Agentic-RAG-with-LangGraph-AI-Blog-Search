...
def set_sidebar():
    ...
    with st.sidebar:
        ...
        qdrant_host = st.text_input("Enter your Qdrant Host URL:", value=os.getenv("QDRANT_HOST", ""), type="password")
        qdrant_api_key = st.text_input("Enter your Qdrant API key:", value=os.getenv("QDRANT_API_KEY", ""), type="password")
        gemini_api_key = st.text_input("Enter your Gemini API key:", value=os.getenv("GEMINI_API_KEY", ""), type="password")
    ...
    for output in graph.stream(inputs):
        for key, value in output.items():
            if key == "generate" and isinstance(value, dict):
                ...
                # If the agent responds directly without using tools
                response = value.get("messages", [None])[0]
